    ...
    try {
      ...
      const rows = balances.map((b: any) => `
        <tr>
          ...
          <td style="padding:8px 12px;border-bottom:1px solid #f0f0f0;text-align:right;">${sym}${Math.round(b.paid||0).toLocaleString("en-IN")}</td>
          <td style="padding:8px 12px;border-bottom:1px solid #f0f0f0;text-align:right;color:${(b.net||0)>=0?"#00A877":"#FF3B30"};">${(b.net||0)>=0?"+":""}${sym}${Math.round(b.net||0).toLocaleString("en-IN")}</td>
        </tr>`).join("");

      const html = `
<!DOCTYPE html><html><head><meta charset="utf-8">
<style>
  body { font-family: -apple-system, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px; color: #111; }
  h1 { font-size: 28px; font-weight: 500; letter-spacing: -1px; margin-bottom: 4px; }
  .sub { color: #888; font-size: 14px; margin-bottom: 32px; }
  .hero { background: #111; color: #fff; border-radius: 20px; padding: 24px; margin-bottom: 24px; }
  .hero-label { font-size: 11px; color: rgba(255,255,255,0.4); letter-spacing: 1.5px; text-transform: uppercase; margin-bottom: 6px; }
  .hero-amount { font-size: 36px; font-weight: 500; letter-spacing: -1px; margin-bottom: 16px; }
  .hero-row { display: flex; gap: 12px; }
  .hero-stat { flex: 1; background: rgba(255,255,255,0.08); border-radius: 12px; padding: 12px; }
  .section-title { font-size: 11px; font-weight: 500; color: #888; letter-spacing: 1.5px; text-transform: uppercase; margin: 24px 0 12px; }
  table { width: 100%; border-collapse: collapse; }
  th { text-align: left; padding: 8px 12px; font-size: 11px; color: #888; font-weight: 500; letter-spacing: 1px; text-transform: uppercase; }
  .ai-box { background: #F5F3FF; border-radius: 16px; padding: 16px; margin: 24px 0; border-left: 3px solid #6D5DFC; }
  .ai-label { font-size: 11px; color: #6D5DFC; font-weight: 500; margin-bottom: 6px; }
  .footer { margin-top: 40px; padding-top: 16px; border-top: 1px solid #f0f0f0; font-size: 12px; color: #aaa; text-align: center; }
</style></head><body>
  <h1>${trip.name}</h1>
  <div class="sub">Generated by Merizo AI · ${new Date().toLocaleDateString("en-IN", { day:"numeric", month:"long", year:"numeric" })}</div>

  <div class="hero">
    <div class="hero-label">Total spent</div>
    <div class="hero-amount">${sym}${Math.round(total).toLocaleString("en-IN")}</div>
    <div class="hero-row">
      <div class="hero-stat"><div style="font-size:10px;color:rgba(255,255,255,0.4);margin-bottom:4px;">Members</div><div style="color:#fff;font-size:16px;font-weight:500;">${balances.length}</div></div>
      <div class="hero-stat"><div style="font-size:10px;color:rgba(255,255,255,0.4);margin-bottom:4px;">Expenses</div><div style="color:#fff;font-size:16px;font-weight:500;">${trip.expense_count||0}</div></div>
      <div class="hero-stat"><div style="font-size:10px;color:rgba(255,255,255,0.4);margin-bottom:4px;">Per person</div><div style="color:#fff;font-size:16px;font-weight:500;">${sym}${Math.round(perPerson).toLocaleString("en-IN")}</div></div>
    </div>
  </div>

  <div class="ai-box">
    <div class="ai-label">✦ Merizo AI Insight</div>
    <div style="font-size:14px;line-height:1.6;">${unsettledBalances(trip)} This report covers all ${trip.expense_count||0} expenses. Average spend per person: ${sym}${Math.round(perPerson).toLocaleString("en-IN")}.</div>
  </div>

  <div class="section-title">Member balances</div>
  <table>
    <thead><tr><th>Name</th><th style="text-align:right;">Paid</th><th style="text-align:right;">Net balance</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>

  <div class="footer">Merizo AI · Smart expense splitting · merizo-app.onrender.com</div>
</body></html>`;
      ...
      if (typeof window !== "undefined") {
        ...
        a.href = url;
        a.download = `${trip.name.replace(/\s+/g,"-")}-report.html`;
        a.click();
        URL.revokeObjectURL(url);
      } else {
        Alert.alert("Report ready", "Full report generated! PDF export available in the next update.");
      }
    } catch (e) {
  ...
  const unsettledBalances = (trip: any) => {
    const txns = trip.settlement_transactions || [];
    if (txns.length === 0) return "All balances are settled. ✓";
    return `${txns.length} payment${txns.length>1?"s":""} needed to fully settle.`;
  };
