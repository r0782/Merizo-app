...
// ── Roboto Mono size and color maps ──────────────────────────────────────────
const ROBOTO_SIZES: Record<DotNumSize, number> = {
  xs:  11,
  sm:  15,
  md:  20,
  lg:  26,
...
const ROBOTO_COLORS_DARK: Record<DotNumColor, string> = {
  indigo: "#7C5CFF",
  white:  "#FFFFFF",
  black:  "#FFFFFF",   // on dark bg, "black" → white
  green:  "#4ADE80",
  red:    "#F87171",
  gold:   "#F5C842",
  muted:  "#555555",
...
const ROBOTO_COLORS_LIGHT: Record<DotNumColor, string> = {
  indigo: "#7C5CFF",   // keep brand color
  white:  "#FFFFFF",
  black:  "#0A0A0A",
  green:  "#0A0A0A",   // light mode → black only
  red:    "#0A0A0A",   // light mode → black only
  gold:   "#0A0A0A",   // light mode → black only
  muted:  "#999999",
};
...
export function SmartNum({
  ...
  const { isDark } = useTheme();
  const colorMap = isDark ? ROBOTO_COLORS_DARK : ROBOTO_COLORS_LIGHT;
  const hexColor = colorMap[color] ?? (isDark ? "#FFFFFF" : "#0A0A0A");
  const fontSize = ROBOTO_SIZES[size] ?? 20;
